...
"""
Attempt to align the system time with the traffic manager time and carla server time.
"""
...
def tutorial(args):
    ...
    try:
        ...
        for frame in range(args.frames):
            ...
            # Draw detection results and calculate average distance for each box
            # 在 YOLO 检测结果处理部分添加以下代码
            if results:
                for result in results:
                    for box, conf, cls_id in zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
                        ...
                        # 画框
                        cv2.rectangle(im_array_bgr, (x1, y1), (x2, y2), (0, 255, 0), 2)

                        # 画标签，防止越界
                        cv2.putText(im_array_bgr, label, (x1, max(10, y1 - 10)),
                        ...
                        if len(box_distances) > 0:
                            avg_distance = np.mean(box_distances)
                            distance_label = f"Avg Distance: {avg_distance:.2f} m"
                            cv2.putText(im_array_bgr, distance_label, (x1, y2 + 20),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                        else:
                            cv2.putText(im_array_bgr, "No LiDAR points", (x1, y2 + 20),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

                # 绘制点云
                for i in range(len(points_2d)):
                    u, v = u_coord[i], v_coord[i]
                    color = color_map[i]  # RGB 格式
                    color_bgr = color[::-1]  # 将 RGB 转换为 BGR
                    im_array_bgr[v, u] = color_bgr  # 绘制单个像素

            # 实时显示图像
            cv2.imshow("CARLA Camera with LiDAR Projection and YOLO Detection", im_array_bgr)
