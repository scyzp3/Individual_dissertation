...
"""
Test for LiDAR and Camera in CARLA vehicle with SFA3D
"""
...
def tutorial(args):
    ...
    # 确保 configs 包含 device 属性
    if not hasattr(configs, 'device'):
        configs.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    model = create_model(configs)
    model.load_state_dict(torch.load(configs.pretrained_path, map_location='cpu'))
    model.to(configs.device)  # 将模型移动到指定设备
    model.eval()
    ...
    try:
        ...
        for frame in range(args.frames):
            ...
            # Change from UE4's coordinate system to an "standard" camera coordinate system.
            ...
            with torch.no_grad():
                # 将点云转换为 SFA3D 输入格式
                points = np.array(p_cloud[:, :3])
                points = torch.from_numpy(points).float().to(configs.device)
                ...
                if len(kitti_dets) > 0:
                    ...
                    im_array_bgr = show_rgb_image_with_boxes(im_array_bgr, kitti_dets, calib)

            # 显示图像
            cv2.imshow("CARLA Camera with SFA3D Detection", im_array_bgr)
