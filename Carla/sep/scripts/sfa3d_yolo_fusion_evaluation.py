...
from data_process.demo2 import Demo_KittiDataset, compute_map_recall
...
if __name__ == '__main__':
    ...
    # Load both detectors so their image-space outputs can be compared and fused.
    ...
    # Camera intrinsic matrix for projecting SFA3D boxes to the RGB image.
    K = np.identity(3)
    K[0, 0] = K[1, 1] = focal
    K[0, 2] = image_w / 2.0
    K[1, 2] = image_h / 2.0
    ...
    with (torch.no_grad()):  
        for sample_idx in range(len(demo_dataset)):
            ...
            # Load detections inputs plus ground-truth image boxes for evaluation.
            ...
            # Evaluate YOLO on target traffic classes only.
            ...
            if len(kitti_dets) > 0:
                # Project SFA3D 3D detections into 2D image boxes for IoU matching.
            ...
            for idx, box in enumerate(boxes):
                # Unmatched YOLO boxes are counted as fusion outputs.
            ...
            # Per-frame object counts help inspect whether detector outputs are aligned.
            print(len(label_box))
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                # Report fused SFA3D/YOLO metrics when the run is stopped manually.
                ...
                break
            if count > 1000:
                # Long-run fallback report for YOLO-only metrics.
