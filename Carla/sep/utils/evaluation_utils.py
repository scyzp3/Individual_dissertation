...
def convert_det_to_real_values_with_scores(detections, num_classes=3):
    """
    将检测结果转换为 KITTI 格式，并返回置信度。

    参数：
    - detections: 3D 检测的结果，列表形式，每个类别有多个检测框
    - num_classes: 类别数

    返回：
    - kitti_dets: (N, 8) 的数组，包含 [类别, x, y, z, h, w, l, yaw]
    - scores: (N,) 的数组，对应置信度
    - labels: (N,) 的数组，存储类别信息
    """
    kitti_dets = []
    ...
    for cls_id in range(num_classes):
        if len(detections[cls_id]) > 0:
            for det in detections[cls_id]:
                # (score-0, x-1, y-2, z-3, h-4, w-5, l-6, yaw-7)
                _score, _x, _y, _z, _h, _w, _l, _yaw = det
                _yaw = -_yaw  # 方向修正
    ...
    return np.array(kitti_dets), np.array(scores), np.array(labels)
